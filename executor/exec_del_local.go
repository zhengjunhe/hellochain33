...
func (t *Hellochain33) ExecDelLocal_SetSelfIntro(payload *pt.SetSelfIntroAction, tx *types.Transaction, receiptData *types.ReceiptData, index int) (*types.LocalDBSet, error) {
	...
	for _, log := range receiptData.Logs {
		if log.Ty == pt.TyLogSetSelfIntroduction {
			...
			set.KV = append(set.KV, &types.KeyValue{Key:key2, Value:nil})
		}
	}
...
func (t *Hellochain33) ExecDelLocal_PingPong(payload *pt.PingPongAction, tx *types.Transaction, receiptData *types.ReceiptData, index int) (*types.LocalDBSet, error) {
	set := &types.LocalDBSet{}
	if receiptData.GetTy() != types.ExecOk {
		return set, nil
	}
